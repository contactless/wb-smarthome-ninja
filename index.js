...
var NoolightDevice = require('./lib/noolite');
var RadioListener = require('./lib/radio_listener');
...
function myDriver(opts,app) {
  ...
  app.on('client::up',function(){
    ...
    // Register a device
    //~ self.emit('register', new Device(247, "Relay 1"));
    self.emit('register', new Device(246, "Relay 2"));


    self.emit('register', new Device(52, "FET 1"));
    self.emit('register', new Device(50, "FET 2"));
    self.emit('register', new Device(57, "FET 3"));
    self.emit('register', new Device(54, "FET 4"));

    self.emit('register', new NoolightDevice("8469"));

    self.radioListener = new RadioListener(opts, app, self);
